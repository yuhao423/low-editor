  ...
  return (
    <div className="w-full h-full flex flex-col bg-white rounded-[24px] overflow-hidden shadow-md">
      ...
      <div className="flex-1">
        <Allotment defaultSizes={showComponentList ? [240, 400, 1, 300] : [240, 1, 300]} className="h-full">
          {/* 左侧物料区 */}
          ...
          {showComponentList && (
            <Allotment.Pane preferredSize={400} minSize={400} maxSize={400}>
              <div className="relative h-full bg-gray-50 border-l border-r border-[#00000010]">
                <button className="absolute top-2 right-2 text-gray-500 hover:text-black" onClick={() => setShowComponentList(false)}>
                  <X className="w-5 h-5" />
                </button>
      ...
      {!showComponentList && (
        <button className="absolute bottom-4 left-4 bg-white px-3 py-1 rounded shadow text-sm border" onClick={() => setShowComponentList(true)}>
          打开组件列表
        </button>
      )}
    </div>
